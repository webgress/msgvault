...
// time on the same message overwrites the prior indexed text.
...
// search index from the messages/bodies/recipients tables.
...
// TestRemoveSourceClearsSearchResults verifies that after RemoveSource,
// searches no longer return that source's messages.
...
// metacharacters don't cause a query error on either backend.
