...
// parseSQLiteTime parses a datetime string from SQLite or PostgreSQL into time.Time.
// Delegates to parseDBTime (defined in sync.go) so the two callers share one
// layout list. Returns the zero time on any parse failure.
