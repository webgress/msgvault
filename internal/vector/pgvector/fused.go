...
func (b *Backend) FusedSearch(ctx context.Context, req vector.FusedRequest) ([]vector.FusedHit, bool, error) {
	...
	if useFTS {
		...
		ctes = append(ctes, fmt.Sprintf(`fts_pool AS (
    SELECT m.id AS message_id,
     ...
     LIMIT %s
		...
		ctes = append(ctes, fmt.Sprintf(`fts_ranked AS (
	...
	if useANN {
		...
		kp1Arg := bind(kPlus1)
		kArg := bind(req.KPerSignal)
		ctes = append(ctes, fmt.Sprintf(`ann_pool AS (
    SELECT e.message_id,
           (e.embedding::vector(%d)) <=> %s::vector AS distance
      FROM embeddings e
      JOIN filtered f ON f.id = e.message_id
     WHERE e.generation_id = %s AND e.dimension = %d
     ORDER BY (e.embedding::vector(%d)) <=> %s::vector
     LIMIT %s
)`, dim, vecArg, genArg, dim, dim, vecArg, kp1Arg))
		ctes = append(ctes, fmt.Sprintf(`ann_ranked AS (
...
func applyFilterClauses(f vector.Filter, bind func(any) string) string {
	...
	for _, term := range f.SubjectSubstrings {
		fmt.Fprintf(&sb, ` AND m.subject ILIKE %s ESCAPE '\'`,
