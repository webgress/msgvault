...
You should see the msgvault tables (`sources`, `participants`,
`conversations`, `messages`, `attachments`, `labels`, `message_labels`,
`message_bodies`, `message_raw`, `message_recipients`,
`conversation_participants`, `reactions`, `participant_identifiers`,
`sync_runs`, `sync_checkpoints`, `collections`, `collection_sources`).
...
Stale binary pre-dating the `JSONPlaceholder` fix. Rebuild: `make install`.
...
PG test runs are slower than SQLite (per-test schema setup overhead), but
the production code paths are the same.
